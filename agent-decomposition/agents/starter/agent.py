...
LEGACY_TOOLS = [
    ...
    "update_erp_record",
    # "send_slack_alert" → notify-templates スキルで代替（サブエージェント不要）
    # "draft_email_to_supplier" → 同上
    # "generate_weekly_report" → weekly-report スキルで代替（コード実行で）
...
SKILLS: list[str] = [
    "notify-templates",   # Cycle 1: テンプレート埋め → サブエージェント不要
    "weekly-report",      # Cycle 1: レポート生成 → Pythonスクリプト1本
    # "reorder-policy",
    # "supplier-selection",
    # "forecasting",
]
...
def build_config(skill_ids: dict[str, str]) -> dict:
    ...
    return {
        ...
        "model": MODEL,
        "system": SHORT_PROMPT + _legacy_tools_note(),  # Cycle 1: 402行 → 15行、詳細はスキルへ
        "tools": tools,
        "skills": [
